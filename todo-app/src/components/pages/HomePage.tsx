...
import { Task, WeeklySlot } from '@/lib/types'
...
function Timer() {
  ...
  const loadPreset = (p: TimerPreset) => {
    setActivePreset(p)
  ...
  const reset = () => {
    setRunning(false)
    setSecondsLeft(activePreset ? activePreset.minutes * 60 : 25 * 60)
  }
    ...
    <div className="rounded-xl border p-4" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
      ...
      <div className="flex justify-center gap-2 mb-3">
        ...
        <button onClick={reset}
          className="w-9 h-9 rounded-full flex items-center justify-center"
          style={{ background: 'var(--morandi-sand)' }}>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {presets.map(p => (
          <div key={p.id} className="flex items-center gap-1 rounded-lg px-2 py-1" style={{ background: activePreset?.id === p.id ? 'var(--morandi-pink)' : 'var(--morandi-sand)' }}>
            <button onClick={() => loadPreset(p)} className="text-xs" style={{ color: activePreset?.id === p.id ? 'var(--morandi-pink-text)' : 'var(--morandi-sand-text)' }}>
              {p.name} {p.minutes}m
            </button>
  ...
  return (
    <div className="rounded-xl border p-4" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => setExpanded(e => !e)} className="p-1 rounded" style={{ border: '0.5px solid var(--card-border)', color: 'var(--morandi-pink-text)' }}>
          <i className="ti ti-arrows-maximize" style={{ fontSize: '10px' }} />
        </button>
      </div>
            ...
            <div key={dateStr} className="flex flex-col items-center py-0.5">
              ...
              {(dCount > 0 || tCount > 0) && (
                <div className="flex gap-0.5 mt-0.5">
                  {dCount > 0 && <div className="w-2.5 h-2.5 rounded-full flex items-center justify-center" style={{ background: 'var(--morandi-pink)' }}>
                    <span style={{ fontSize: '6px', color: 'var(--morandi-pink-text)', fontWeight: '600' }}>{dCount}</span>
                  </div>}
                  {tCount > 0 && <div className="w-2.5 h-2.5 rounded-full flex items-center justify-center" style={{ background: 'var(--morandi-sand)' }}>
                    <span style={{ fontSize: '6px', color: 'var(--morandi-sand-text)', fontWeight: '600' }}>{tCount}</span>
                  </div>}
                </div>
              )}
            </div>
      ...
      <div className="flex gap-3 mt-3 pt-3" style={{ borderTop: '0.5px solid var(--divider)' }}>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full" style={{ background: 'var(--morandi-pink)' }} />
          <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Deadline</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full" style={{ background: 'var(--morandi-sand)' }} />
      ...
      {/* Expanded modal */}
      {expanded && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={() => setExpanded(false)}>
          <div className="rounded-2xl shadow-2xl p-6 w-full max-w-lg" style={{ background: 'var(--card-bg)' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <span className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
                {new Date(currentMonth.year, currentMonth.month).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
              </span>
              <button onClick={() => setExpanded(false)} style={{ color: 'var(--text-muted)' }} className="text-xl">&times;</button>
            </div>
            <div className="grid grid-cols-7 text-center mb-2">
              {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d => (
                <span key={d} className="text-xs font-medium" style={{ color: 'var(--text-muted)' }}>{d}</span>
              ))}
            </div>
            <div className="grid grid-cols-7 gap-1">
              {days.map((dateStr, i) => {
                if (!dateStr) return <div key={i} />
                const { deadlines: dl, tasks: tl } = getItemsForDay(dateStr)
                const isToday = dateStr === today
                return (
                  <div key={dateStr} className="rounded-lg p-1.5 min-h-[60px]" style={{ background: isToday ? 'var(--morandi-pink)' : 'var(--bg)', border: '0.5px solid var(--divider)' }}>
                    <span className="text-xs font-medium block mb-1" style={{ color: isToday ? 'var(--morandi-pink-text)' : 'var(--text-secondary)' }}>
                      {new Date(dateStr + 'T00:00:00').getDate()}
                    </span>
                    {dl.map((d: any) => (
                      <div key={d.id} className="text-xs px-1 rounded mb-0.5 truncate" style={{ background: 'var(--morandi-pink)', color: 'var(--morandi-pink-text)' }}>
                        {d.subject}: {d.task}
                      </div>
                    ))}
                    {tl.map((t: Task) => (
                      <div key={t.id} className="text-xs px-1 rounded mb-0.5 truncate" style={{ background: 'var(--morandi-sand)', color: 'var(--morandi-sand-text)' }}>
                        {t.task}
                      </div>
                    ))}
                  </div>
...
export default function HomePage({ user, onTaskChange }: Props) {
  ...
  const fetchAll = async () => {
    ...
    const todayDeadlines = deadlines.filter(d => d.due_date === todayStr).length
    setTodayStats({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--card-bg)', borderColor: 'var(--card-border)' }}>
        <div className="px-4 py-3 flex items-center justify-between" style={{ background: 'var(--section-header)', borderBottom: '0.5px solid var(--divider)' }}>
        ...
        {todayTasks.map(task => {
          ...
          return (
            <div key={task.id} className="flex items-center gap-2 px-4 py-2.5 border-b" style={{ borderColor: 'var(--divider)', opacity: isDone ? 0.6 : 1 }}>
              <button
              ...
              <span className="flex-1 text-sm min-w-0 truncate" style={{ color: isDone ? 'var(--text-muted)' : 'var(--text-primary)', textDecoration: isDone ? 'line-through' : 'none' }}>
                {task.task || 'Untitled'}
              </span>
              {task.due_date && (
                <span className="text-xs flex-shrink-0" style={{ color: days !== null && days <= 2 ? 'var(--urgent-today-text)' : 'var(--text-muted)' }}>
                  {getShowsAs(task.due_date)}
                </span>
              )}
              <div className="text-xs px-1.5 py-0.5 rounded-full flex-shrink-0" style={{ background: isDone ? 'var(--morandi-sage)' : 'var(--morandi-pink)', color: isDone ? 'var(--morandi-sage-text)' : 'var(--morandi-pink-text)' }}>
                {task.progress}
              </div>
